...
for (input_index in 1:length(user_input$age_range_start)) {
  ...
  # Take random sample from policies generated in ROI adjusted profit table
  ...
  for (year in 1:years){
    ...
    for (month in 2:13){
      ...
      # For each month, calculate the benefit payout value
      ...
      for (j in 1:nrow(fund_policies)){
        ...
        if (isTRUE(matured) && isTRUE(not_dead)) {                                                         # if yes to both, add payout value
          one_month_payout = (one_month_payout + monthly_annuity)
          payouts = payouts + 1
        }
        if(isFALSE(not_dead)){
          deaths = deaths + 1
      ...
      # Sell n premiums per month, add to fund_policies table
      # TODO, how many policies should be sold per month?
      sales_goal = 10
